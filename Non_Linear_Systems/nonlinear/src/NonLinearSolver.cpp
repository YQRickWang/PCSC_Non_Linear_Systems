...
double* NonLinearSolver::LinearSolver_LU(double **A, double *b)
{
    ...
    LUDecomposition(A,L,U,dim);//perform LUDecomposition
    y = Forward(L,b,dim);//perform forward method
    x = Backward(U,y,dim);//perform backward method
...
{
    zeroPoint.clear();//clear zeroPoint map
}